...
`,
    ...
    tableStructure: [
        ...
        },
        {
            ...
            initNode: '检查上文中是否存在上述类型的关键事件。有则插入一条，绝大多数情况下无事件需要记录，直接留空即可。', insertNode: '仅当本轮回合中发生了上述类型的关键事件时才insertRow。绝大多数回合不需要插入——日常对话、情绪波动、氛围描写、闲聊讨论均不是关键事件。如果无法确定是否属于关键事件，则默认不插入。示例：insertRow(4, {0:"角色A", 1:"向<user>表白", 2:"日期", 3:"地点", 4:"紧张"})', updateNode: '仅当同一事件的后续发展产生了决定性新信息时才updateRow。禁止为措辞润色、补充情绪细节、或追加非关键信息。如果不确定是否需要更新，则默认不操作。', deleteNode: '当同一时间段存在3条以上记录时，保留最具代表性的一条，删除其余。',
        },
        {
